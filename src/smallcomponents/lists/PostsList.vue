<template>
    <div class="postpage">
        <div class="searchpost">
            <div class="postsearch">
                <div class="postsearch-header">
                    <h3>Post</h3>
                </div>
                <div class="postsearch-search">
                    <input @keyup.enter="filterPostById" type="text" placeholder="Search Post (ID) ">
                    <button>Search</button>
                </div>
            </div>
        </div>
        <div class="allpost">
            <div v-for="post in allPosts" :key="post.id" class="eachpost">
                <div class="postimg">
                    <img src="data:image/png;base64," alt="postimg">
                </div>
                <div class="postdetail">
                    <router-link class="link" :to="{name:'PostDetail',params:{id:post.id}}"><h3>{{post.title}}</h3></router-link>
                    <p>{{post.body}}</p>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import { mapActions, mapGetters } from 'vuex'
export default {
    name:"PostsList",
    computed: mapGetters(["allPosts"]),
    created(){
        this.fetchPosts()
    },
    methods:{
        ...mapActions(["fetchPosts","getPost","deletePost","updatePost"]),
        filterPostById(e){
            if(e.target.value === ""){
                this.fetchPosts()
            }else{
                this.getPost(e.target.value)
            }
        }
    },
}
</script>

<style>
.postimg img{
    height: 100px;
    width: 160px;
    margin-right: 10px;
}
.link{
    text-decoration: none;
    color:black;
}
.link h3{
    font-weight: 400;
}
.eachpost{
    display: flex;
    align-items: center;
    font-size: small;
    font-family: "Poppins",sans-serif;
    height: fit-content;
    width: 470px;
}
.allpost{
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    height: 450px;
    overflow-y: scroll;
}
.postdetail p{
    font-weight: 100;
}
.postsearch-header{
    display:flex;
    align-items: center;
    font-family: "Poppins",sans-serif;
    padding:25px;
    box-shadow: 0 0 1px rgba(0,0,0,.5);
}
.postsearch-header h3{
    font-weight: 300;
    letter-spacing: 5px;
}
.postsearch-search{
    padding:8vh;
    display:flex;
}
.postsearch-search input{
    height: 35px;
    width: 250px;
    outline: none;
    border-radius: 20px;
    border:none;
    box-shadow: 0 2px 2px rgba(0,0,0,.5);
    padding-left: 20px;
}
.postsearch-search button{
    margin-left: 5vh;
    border-radius: 20px;
    width: 80px;
    border:none;
    box-shadow: 0 2px 2px rgba(0,0,0,.5);
    outline: none;
    background-color: white;
    color:grey;
}
</style>